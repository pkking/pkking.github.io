---
name: blog-writer
description: "技术博客深度写作。不是内容生成器，是观点表达工具。强制形成判断 → 概念先行/术语角标 → 中英双版 → mermaid/architecture 优先画图 → SEO + FAQ + 分发。触发词：写博客、写文章、blog、介绍新工具、参考这个链接写、整理资料成文。使用此 skill 当用户需要：(1) 撰写技术博客 (2) 整理资料/参考 URL 写深度文 (3) 介绍新工具/新技术 (4) 产出带架构图/流程图的技术文章"
---
