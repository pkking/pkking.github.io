---
name: blog-writer
description: "技术博客深度写作。不是内容生成器，是观点表达工具。强制形成判断 → article-spec.yml 固化上下文 → 概念先行/术语角标 → 中英双版 → mermaid/architecture 优先画图 → SEO + FAQ + 分发。触发词：写博客、写文章、blog、介绍新工具、参考这个链接写、整理资料成文。使用此 skill 当用户需要：(1) 撰写技术博客 (2) 整理资料/参考 URL 写深度文 (3) 介绍新工具/新技术 (4) 产出带架构图/流程图的技术文章"
---
...
步骤4 撰写 → 步骤5 封面图+配图 → 步骤6 质量门禁（⛔） → 步骤7 发布+分发
```
...
- `structure`：H2 级章节计划和每章要回答的问题
- `regeneration_policy`：重生成时哪些必须保留、哪些可以重写、是否刷新 `date`
- `revision_log`：每次实质修改的原因和日期

最小模板见 [references/article-spec-template.yml](references/article-spec-template.yml)。如果现有文章没有 spec，必须先从正文、front matter 和参考链接反推一份，再继续修改文章。

**质量要求：**
- spec 是机器可读 YAML，不写成散文
- `core_claim` 和步骤 3.1 一致
- `sources` 至少覆盖正文参考里的主要一手来源
